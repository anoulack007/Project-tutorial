...
exports.registerAcc =async (request,response)=>{
    const{userName,password,firstName,surName,age,address,email} = request.body
...
exports.ReadAcc = async(request,response)=>{
    const account = await acc.find()
    response.status(200).send(account);
}
exports.ReadAcc1 = async(request,response)=>{
    const {id} = request.params
    const account = await acc.findById(id)
    response.status(200).send(account);
}
exports.UpdateAcc = async(request,response)=>{
    const { id } = request.params;
    const {userName,is_online,is_active_status} = request.body;
    // ດັກໄອດີ 
    const duct = await acc.findOne({userName});
    if (duct) {
    return response.status(409).send("User Mee UserName ny leo");}
    const account = await acc.findByIdAndUpdate(id, {userName,is_online,is_active_status}, { new: true });
